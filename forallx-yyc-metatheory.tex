...
\begin{align*}
	...
	& (\enot(R \eand \fbox{\enot(\fbox{S} \eand \fbox{T})}) && \text{DeM}\\
	& (\enot(R \eand (\enot \fbox{S} \eor \enot \fbox{T})) && \text{DeM}
\end{align*}
(We've highlighted the sentence replaced, and those mathching the $\metav{P}$ and~$\metav{Q}$ in De Morgan's Laws for clarity, but this is not necessary.)
